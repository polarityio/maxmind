...
polarity.export = IntegrationBlock.extend({
  ...
  notificationGroupingKey: Ember.computed(
    'data.details.network',
    'entity.value',
    'maxmindMergeNetworks',
    'ipAddresses.@each',
    function() {
      let key;
  ...
  belongsToNotification: function(notificationGroupingKey, entityValues, entityNotificationsDict) {
    ...
    if (this.get('maxmindMergeNetworks') === true) {
      if (
        (this.get('data.details.network') === network &&
          entityNotificationsDict.has(this.get('entity.value')) === false) ||
  ...
  onNotificationWindowBlockInserted: function(entityNotification, integrationBlock) {
    ...
    if (
      integrationBlock.get('type') === this.get('type') &&
